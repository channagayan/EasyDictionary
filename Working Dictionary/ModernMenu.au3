...
If Not IsDeclared("MIIM_SUBMENU")			Then Global Const $MIIM_SUBMENU = 0x00000004
			...
			If $nID = $nMenuItemID And $bHasSide Then
				...
				If $hSideImage = 0 Then
					...
					DrawTextW($hDC, DllStructGetPtr($stSideText), _
									...
									BitOr($DT_BOTTOM, $DT_SINGLELINE, $DT_NOCLIP))
				...
				Else
					...
					If $bStretch Then
						StretchBlt($hDC, 0, 0, $nSideWidth, $arIR[3], $hCDC, 0, 0, $nSideWidth, $nSideHeight, $SRCCOPY)
					Else
						BitBlt($hDC, 0, 0, $nSideWidth, $arIR[3], $hCDC, 0, $nSideHeight - $arIR[3], $SRCCOPY)
					EndIf
	...
	If $bIsBitmap Then
		...
		If $sResName = "" Then
			$hBitmap = LoadImageW(0, DllStructGetPtr($stFile), 0, 0, 0, _
							BitOr($LR_LOADFROMFILE, $LR_DEFAULTSIZE, $LR_CREATEDIBSECTION, $LR_LOADTRANSPARENT))
		Else
			...
			If $hLib <> 0 Then
				If IsNumber($sResName) Then
					$hBitmap = LoadImageW($hLib, $sResName, 0, 0, 0, _
									BitOr($LR_DEFAULTSIZE, $LR_CREATEDIBSECTION, $LR_LOADTRANSPARENT))
				Else
...
Func MulDiv($nInt1, $nInt2, $nInt3)
	$nResult = DllCall($hKernel32Dll, "int", "MulDiv", _
											"int", $nInt1, _
											"int", $nInt2, _
											"int", $nInt3)
	Return $nResult[0]
...
Func SelectObject($hDC, $hObj)
	Local $hResult = DllCall($hGdi32Dll, "hwnd", "SelectObject", _
												"hwnd", $hDC, _
												"hwnd", $hObj)
	Return $hResult[0]
EndFunc
